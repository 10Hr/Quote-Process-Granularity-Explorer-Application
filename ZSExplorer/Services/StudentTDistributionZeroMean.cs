...
public class StudentTDistributionZeroMean
{
    ...
    public StudentTResult StudentT(double[] data)
    {
        ...
        const double maxReasonableNu = 100.0;
        ...
        // If sample kurtosis < 3, t-distribution cannot fit (t has kurtosis >= 3)
        if (k < 3.0)
        {
            throw new InvalidOperationException($"Sample kurtosis ({k:F2}) is less than 3.");
        }

        Func<double, double> f = nu =>
        {
            if (nu <= 4) return double.PositiveInfinity;
            double theoreticalKurtosis = 3.0 * (nu - 2) / (nu - 4);
            return theoreticalKurtosis - k;
        };

        double nu = RegulaFalsi(f, 4.1, maxReasonableNu, 1e-6, 100);
        
        // Cap the result
        if (nu >= maxReasonableNu || double.IsInfinity(nu))
        {
            return new StudentTResult
            {
                Location = mu,
                Scale = Math.Sqrt(s2),
                DegreesFreedom = double.PositiveInfinity,
            };
        }
